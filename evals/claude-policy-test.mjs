...
// The workflow is the executable source, so the command text must name the model it dispatches.
check('normal review uses one Opus and one fixed astra outsider', /one independent Opus xhigh reviewer/.test(review) && review.includes('gpt-6-astra') && !review.includes('gpt-5.6-sol') && reviewWorkflow.includes('codex-gpt-6-astra-low') && /15–25-agent council/.test(review) && /one Codex outsider/.test(review))
check('review demands the smallest-thing rule on the production diff', /smallest-thing rule/.test(review) && /no caller reads yet/.test(review) && /rules\/principles\.md/.test(review))
...
check('Codex review wrapper pins astra low by default and Sol xhigh behind -X', codexRun.includes('REVIEW_MODEL="gpt-6-astra"') && codexRun.includes('REVIEW_EFFORT="low"') && /-X\) REVIEW_MODEL="gpt-5\.6-sol"; REVIEW_EFFORT="xhigh"/.test(codexRun) && codexRun.includes('--sandbox read-only') && !/--model gpt-5\.6-sol/.test(codexRun))
...
// A char bound drifts with wording, so anchor on the section that owns the contract.
