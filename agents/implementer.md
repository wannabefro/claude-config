...
5. Inspect `git status --short` and `git diff --stat` or `git diff --name-only`
   ...
   shows nothing changed, return `failed` and quote the verification output. A
   clean exit code is not evidence that work happened.
6. Return the structured handoff below. State any unavailable CLI or runtime.

## Safety rules

- Do not call another agent or skill.
- Only output from a command you ran this turn is evidence. A tool's or
  another agent's success message does not count.
